...
// GET: All tuitions (optional query for studentEmail)
...
// ✅ Create new payment (Stripe success এর পর)
app.post('/payments', async (req, res) => {
  ...
  payment.status = 'Paid'; // default status
...
// ✅ Get all payments of a specific student
app.get('/payments/student/:email', async (req, res) => {
  const { email } = req.params;
  const result = await paymentsCollection.find({ studentEmail: email }).sort({ createdAt: -1 }).toArray();
  res.send(result);
});

// ✅ Get all payments of a specific tutor (revenue history)
app.get('/payments/tutor/:email', async (req, res) => {
  const { email } = req.params;
  const result = await paymentsCollection.find({ tutorEmail: email, status: "Paid" }).sort({ createdAt: -1 }).toArray();
  res.send(result);
});
